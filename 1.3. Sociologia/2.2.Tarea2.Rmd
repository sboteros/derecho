...
```{r fuentes, include=FALSE}
    ...
    # Guarde el archivo correspondiente en ./data/
    ...
    # Guarde tanto el archivo de indicadores como el de descriptores en la carpeta
    # correspondiente.
...
habitantes[^Poblacion] ubicado en las Sierras Orientales de Veracruz, cerca de
Orizaba. Esta ubicación, al igual que el hecho de que más del 
`r floor((51.29+30.13)/10)*10`% del territorio esté cubierto por bosque y selva,
y que el municipio se encuentre bañado por el río Blanco, en la cuenca de
...
`r pobreza %>% filter(clave_municipio == 30074) %>% filter(indicador == 'poblacion') %>% pull(valor) %>% formato` 
habitantes. De estos, 130 eran hablantes de lenguas
indígenas (Náhuatl y Mazateco). \autocites{data:coneval, otro:huiloapan}.

Económicamente, Huiloapan es un municipio rezagado, en el que un 
`r pobreza %>% filter(clave_municipio == 30074 & indicador == 'pobreza') %>% pull('valor')`%
de la población se encuentra en situación de pobreza y un 
...
que el municipio se encuentra por debajo de la media; por ejemplo, en términos de
pobreza se ubica en la posición 
...
`r pobreza %>% pull(clave_municipio) %>% unique %>% length %>% formato` municipios. En la 
tabla \ref{tab:coneval} se observan los principales indicadores para el 
municipio.\autocite[Elaboración propia,][]{data:coneval}
...
Huiloapan no es un municipio particularmente violento, o al menos no uno en 
donde los hechos violentos se investiguen por las autoridades: durante el año
2019 se abrieron allí 
...
proporción con el  número de sus habitantes. A manera de referencia, en la tabla 
\ref{tab:carpetas} se muestra la proporción de carpetas de investigación abiertas
por cada 100 habitantes durante el 2019 en los diez municipios con mayor 
proporción.\autocites[Elaboracion propia,][]{data:coneval, data:seguridad}
...
Sin embargo, los principales delitos por los que se abrieron investigaciones
durante el año 2019  se relacionan con hechos violentos, tales como robo,
violencia familiar y homicidio. En la tabla \ref{tab:delitos_huiloapan} se 
observan los 10 tipos de delitos con mayor número de investigaciones 
abiertas en el municipio.\autocite[Elaboración propia,][]{data:seguridad}
...
vehículo cuyas señas de desconocen. Los elementos policiales se demoraron unas
dos horas en llegar al lugar para practicar el levantamiento del
cadáver.\autocites{prensa:feminicidionatali1, prensa:feminicidionatali2} Las
imágenes de *Google Maps* no llegan hasta el lugar donde fue asesinada, muy cerca
a una gasolinería de Pemex y a menos de 500 metros de una estación de la Policía
Federal que se ubica sobre la Autopista Ciudad Mendoza -- Orizaba; sin embargo,
la calle se ve sin pavimentación, solitaria, y hay que caminar por un lugar sin 
iluminación y con unos muros enormes sin ventanas.[^Nota]
...
mujeres durante el año pasado. En la tabla \ref{tab:feminicidas} se señalan los 
municipios con mayor número de feminicidios en el país durante el año 
pasado.\autocites[únicamente se consideran en este cálculo a los municipios con 5,000 habitantes o  más. Elaboracion propia,][]{data:coneval, data:seguridad}
